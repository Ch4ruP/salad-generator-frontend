...
export default function FoodGroup(props: foodGroupProps) {
    ...
    function getCurrentIngredientsAsStrings() {
        return foodEl?.filter(x => x.name !== "No Options" && x.name !== undefined).map((el) => el.name)
    }
    ...
    async function getIngredients(foodGroup: string): Promise<Object[]|null> {
        try {
            ...
            const response = await axios.post('http://localhost:5432/getFiveIngredients', {data: userData})
